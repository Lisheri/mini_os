...
static HELLO: &[u8] = b"Hello World!"; // 定义字节字符串类型的静态变量
...
pub extern "C" fn _start() -> ! {
    ...
    // 迭代 HELLO 的每个字节, 同时使用 enumerate() 获得一个额外的序号变量 i
    for (i, &byte) in HELLO.iter().enumerate() {
        unsafe {
            // 使用 offset 偏移裸指针, 解引用裸指针将字符串的每个字节和对应的颜色字节 -- 0xb(青色) 写入对应的内存位置
...
// + ------------ 创建最小操作系统内核 --------------
// 它将向显示器打印字符串, 并能被打包为一个能够引导启动的磁盘映像

// ! 下面开始真正的创建一个最小内核, 目标就一个: 创建一个内核的磁盘映像, 能够在启动时, 向屏幕输出一行 "hello world"
